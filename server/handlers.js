...
const deleteRecipe = async (req, res) => {
    ...
    try {
        ...
        result
            ? res.status(200).json({ status: 200, data: "Recipe deleted successfully" })
...
const updateRecipe = async (req, res) => {
    ...
    try {
        ...
        let newValues = { $set: { name: name, description: description, ingredients: ingredients, instructions: instructions } }

        const result = await db.collection("recipes").updateOne(query, newValues);

        result
            ? res.status(200).json({ status: 200, data: req.body, message: "Recipe is updated" })
            : res.status(404).json({ status: 404, data: req.body, message: "Recipe is not found" });
    }
